...
public class DuplicatedResourcesMerger {
    ...
    private void generateDuplicatedLog(File logFile, BundleModule bundleModule) throws IOException {
        ...
        for (Map.Entry<ZipPath, String> entry : duplicatedFileList.entrySet()) {
            ...
            System.out.printf("[merge duplicated] found duplicated file, path: %s%n", bundleModule.getName().getName() + "/" + entry.getKey().toString());
        ...
        writer.close();
        System.out.printf(
                "[merge duplicated] duplicated count %s, total size: %s%n",
